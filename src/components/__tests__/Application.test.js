...
it("loads data, cancels an interview and increases the spots remaining for Monday by 1", async () => {
  ...
  const appointment = getAllByTestId(container, "appointment").find((each) =>
    queryByAltText(each, "Delete")
  );
...
it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
  ...
  const day = getAllByTestId(container, "day").find(
    (day) => queryByText(day, "Monday") //check DayListItem for text "Monday" and text "1 spot remaining"
  );
...
it("shows the save error when failing to save an edited appointment", async () => {
  ...
  await waitForElement(() =>
    getByText(container, "Save Error. Please contact us @ 888-8888.")
  ); //wait for error msg to display
...
it("shows the delete error when failing to delete an existing appointment", async () => {
  ...
  await waitForElement(() =>
    getByText(appointment, "Delete Error. Please contact us @ 888-8888.")
  );
